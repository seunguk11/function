...
st.title("2. 최고차항의 계수")
st.write("a값이 바뀌면 포물선의 넓이와 방향이 바뀝니다.")
...
ax.annotate("꼭짓점 (0,0)", xy=(0, 0), xytext=(2, np.max(y) * 0.1 if a > 0 else np.min(y) * 0.5), arrowprops=dict(arrowstyle="->", color="red"))
ax.set_xlim(-10, 10)
ax.set_ylim(min(np.min(y), -5), max(np.max(y), 5))
ax.set_xlabel("x")
ax.set_ylabel("f(x)")
...
st.write("- 꼭짓점은 항상 (0, 0)이며 y절편과 x절편도 0입니다.")
